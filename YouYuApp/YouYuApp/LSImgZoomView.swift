...
class LSImgZoomView: UIView,UIScrollViewDelegate {
    ...
    init(baseframe: CGRect) {
        ...
        let doubleTap = UITapGestureRecognizer(target: self, action: #selector(LSImgZoomView.actionOfDoubleTap(_:)))
        doubleTap.numberOfTapsRequired = 2
        doubleTap.numberOfTouchesRequired = 1
        zoomview!.addGestureRecognizer(doubleTap)
        
        //关闭
        let closeTap = UITapGestureRecognizer(target: self, action: #selector(LSImgZoomView.actionOfCloseTap(_:)))
